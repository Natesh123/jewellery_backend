...
const getAllQuotations = async (req, res, next) => {
  try {
    ...
    const { quotations, total } = await quotationService.getAllQuotations(user.branch_id, {
      page: parseInt(page),
      limit: parseInt(limit),
      search,
      metal,
      status,
      start_date, // Make sure this is here
      end_date    // Make sure this is here
    });
...
const updateQuotation = async (req, res, next) => {
  try {
    ...
    const updatedQuotation = await quotationService.updateQuotation(id, updateData);
...
const deleteQuotation = async (req, res, next) => {
  try {
    ...
    const uploadDir = path.join(__dirname, '../../public/uploads/quotations', id);
    if (fs.existsSync(uploadDir)) {
